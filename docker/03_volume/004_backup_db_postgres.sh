...
docker run -d --name postgres_container -e POSTGRES_PASSWORD='MyPasswd' postgres:16
...
docker exec -t postgres_container pg_dump -U postgres -d mydatabase > mydatabase_backup.sql
